...
import { Calendar, Info, Loader2, Plus, PowerOff, UserPlus, X } from "lucide-react";
...
import { jobApi, useMyJobs } from "../../job/api/job.api";
...
export const EmployerManageJobs = () => {
  ...
  const refreshJobs = () => queryClient.invalidateQueries({ queryKey: ["my-jobs"] });
  ...
  const getAvailableMembers = (job: any) => {
    const assignedIds = new Set(job.assignees?.map((item: any) => item.userId) || []);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium ml-4">
            Owner quản lý toàn bộ tin, member làm việc theo phạm vi được phân công.
          </p>
        </div>
        <Button
        ...
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="font-black text-slate-800">Phụ trách</TableHead>
                    ...
                    <TableCell className="py-5 min-w-[260px]">
                      <div className="flex flex-wrap gap-2">
                        {job.assignees?.length ? (
                            ...
                            >
                              ...
                              {user?.isOwner && (
                                <button
                                  ...
                                  onClick={() => handleUnassign(job.id, assignee.userId)}
                                >
                                  <X size={12} />
                                </button>
                              )}
                          ...
                          <span className="text-xs text-slate-400 font-bold">Chưa phân công</span>
                        )}
                      </div>
                      ...
                      {user?.isOwner && (
                        <div className="mt-3 flex items-center gap-2">
                          <Select
                            ...
                            onValueChange={(memberId) => handleAssign(job.id, memberId)}
