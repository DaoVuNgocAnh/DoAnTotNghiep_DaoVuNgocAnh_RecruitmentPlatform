...
export const JobCard = ({ job }: JobCardProps) => {
  return (
    <div className="relative group">
      <Link to={`/jobs/${job.id}`}>
        <Card className="hover:shadow-[0_20px_50px_rgba(0,177,79,0.15)] transition-all duration-500 border-slate-100 rounded-[2rem] overflow-hidden cursor-pointer bg-white active:scale-[0.98] border-2 hover:border-[#00b14f]/20">
      ...
      {/* Save Button: Absolute positioned to avoid Link triggering */}
      <div className="absolute top-8 right-8 z-10">
        <SaveButton targetId={job.id} targetType="JOB" className="bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-md" />
      </div>
    </div>
  );
};